...
#if defined(NDEBUG)
# error "Uselesscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x18e0ae67dbc51c4ada04933fe544a127f4893410a4b2d54b5c0e8aea1c233053");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Uselesscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Uselesscoin Signed Message:\n";
...
{
    // Uselesscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // S
    nSubsidy >>= (nHeight / 5); // Uselesscoin

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2* 24 * 60 * 60; // Uselesscoin: 
static const int64 nTargetSpacing = 1 * 60; // Uselesscoin: 
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Uselesscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Uselesscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x13c2c4551d6bcd811a2cc6cebb7549c55ec441e9fcddf771e167bc2d05a8798c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "useless 09/10/2021";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045c0eb0b84d80eb997dcb6885b76dc104d8b6b38e098635344c50b8950d964fb11c92d18c5f397d3a2da3eeecbe32b3362375f8ef5fa8ee57a3133bb41e778b4f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1633804111;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086688627;

        if (fTestNet)
        {
            block.nTime    = 1633804082;
            block.nNonce   = 386376383;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x3ef59dff35a5528fbfb730b4013a3c9c36ae49a7985cb7fe0bf6c37bc45ea2bd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc5, 0xb7, 0xd1 }; // Uselesscoin.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UselesscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UselesscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UselesscoinMiner(CWallet *pwallet)
{
    printf("UselesscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("uselesscoin-miner");
    ...
    try { loop {
        ...
        printf("Running UselesscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UselesscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UselesscoinMiner, pwallet));
}
